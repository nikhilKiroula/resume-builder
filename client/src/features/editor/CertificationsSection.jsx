...
const CertificationsSection = () => {
  ...
  const addItem = () => setItems([...items, { ...EMPTY, _id: Date.now().toString() }]);
  ...
  return (
    <EditorSection title="Certifications" icon={Award} badge={items.length || undefined}>
      <div className="space-y-3">
        ...
        {items.map((cert, idx) => (
          <div key={cert._id || idx} className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 bg-white dark:bg-slate-900 space-y-2">
